...
model_formula <- 
  ...
  aux(ever ~ fulltime == 1 | lag(ever) == 1 | init(0)) +
  splines(df = 6, noncentered = TRUE)

fit <- dynamite(
  model_formula, data = d, group = "id", time = "age",
...
coefs <- coef(fit, type = "delta", probs = c(0.025, 0.1, 0.9, 0.975)) |> 
  filter(time > 20) |> 
    mutate(
      ...
      "delta_fulltime_genderWoman:ever_lag1" = "Woman",
      "delta_fulltime_genderMan:ever_lag1" = "Man",
    ...
    Coefficient = recode(parameter,
        ...
        "delta_fulltime_genderWoman:ever_lag1" = "lag(ever)",
        "delta_fulltime_genderMan:ever_lag1" = "lag(ever)",
